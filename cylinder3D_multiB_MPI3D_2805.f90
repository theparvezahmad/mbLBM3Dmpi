...
integer,parameter,dimension(3)::	snapList=[10,100000,200000]
...
double precision:: Cd, Cs, Cl, wt(0:q), rhoAvg, rhoAvgAll, xiL(3), yiL(3)
!coarse variables
...
!if(cylinderId) print*,id," is cylinderId"
!stop
!-----------------------------------------------------------------------
if(cylinderId) then
...
!-----------------------------------------------------------------------
	...
	if (masterId .and. mod(ts,freqDisp) == 0) then
		rhoAvgAll =rhoAvgAll/(product(nP-2)-(eastFace-westFace-3)*(topFace-bottomFace-3)*(nP(2)-2))
		write(*,'(A,I8,2(1X,F8.4))') "Rho(Coarse) ",	ts, rhoAvgAll, rho_c(5,5,5)
	endif
!----------------------------------------------------------------------
...
!~ 			enddo
	...
	!-----------------------------------------------------------------------
		...
		!endif	!if(cylinderId) ends
			...
			rhoAvg = rhoAvg/((nx_f-4)*(ny_f-2)*(nz_f-4)) 
	!----------------------------------------------------------------------
		!if(cylinderId)
			do k=3,nz_f-2 !Collision
				do j=2,ny_f-1
					do i=3,nx_f-2
	...
	!----------------------------------------------------------------------
			...
			Cd = fx_t/(haf*rho0*uInlet*uInlet*D*(nP(2)-2)) 
			Cl = fy_t/(haf*rho0*uInlet*uInlet*D*(nP(2)-2)) 
			Cs = fz_t/(haf*rho0*uInlet*uInlet*D*(nP(2)-2)) 
	!--------------------------------------------------------------------		
			if (mod(ts,freqDisp) == 0 .and. iter==m) then
				write(10,'(I8,1X,4(F10.4,1X))') ts, rhoAvg, Cd, Cs, Cl
				write(*,'(A,I8,2(1X,F8.4))') "Rho(fine  ) ",ts, rhoAvg, rho_f(nx_f-3,ny_f-3,nz_f-3)
			endif
	!----------------------------------------------------------------------
			fx(1) = fx(2) 
			fy(1) = fy(2)
				...
				do j=2,ny_f-1
					do i=2,nx_f-1
						write(12,'(3(F8.2,1X),I4,1X,4(F8.4,1X))') (westFace+dble(i)/m),(1+dble(j)/m),(bottomFace+dble(k)/m),&
																											 isn(i,k),ux_f(i,j,k),uy_f(i,j,k),uz_f(i,j,k),rho_f(i,j,k) 
					enddo
					write(12,*)
				enddo
				write(12,*)
