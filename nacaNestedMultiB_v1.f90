...
implicit none
!-----------------------------------------------------------------------
integer,parameter,dimension(3)::	topo		=[6,1,5]
																	
integer,parameter,dimension(3)::	snapList=[1,50000,100000]
...
double precision,parameter:: 	Chord=20.0d0	,&
															tauC=0.56d0		,&
															rho0=1.0d0		,&
															uInlet=0.08d0	,&
...
logical 	:: myperiod(3), bottomId, topId, eastId, westId, northId, southId, masterId, sphereId
...
masterId=(id==0)
!=======================================================================
z(3)%np=[999,21,121] !must be odd

z(1)%span=[26,z(3)%np(2)-2-1,6]! -2 to exclude buffer,-1 becoz span is 1 less than ny
z(2)%span=[32,z(3)%np(2)-2-1,10]!span should be even

z(2)%np=m(2)*z(2)%span+3 !includes 2 buffer nodes
z(1)%np=m(1)*z(1)%span+3

xPosObj=416
z(3)%centr(1)  =xPosObj-Chord/two
z(3)%centr(2:3)=z(3)%np(2:3)/2+1 !centr(2) will be unused
z(2)%centr(1)  =z(2)%np(1)/2+1-m(2)*Chord/two !z(2) unused
z(2)%centr(2:3)=z(2)%np(2:3)/2+1
z(1)%centr(1)  =z(1)%np(1)/2+1-m(1)*Chord/two
z(1)%centr(2:3)=z(1)%np(2:3)/2+1
...
if(isDebug .and. masterId) then
	write(*,*) "Re: ",3.0d0*Chord*uInlet/(z(3)%tau-haf)
	write(*,*) "----------------------"
	write(*,*) "grid ratio    :",m
...
do k=1,3
	...
	tmpI1=z(3)%np(k)/myDim(k)
	tmpI2=myDim(k)-mod(z(3)%np(k),myDim(k))
...
enddo
!-----------------------------------------------------------------------
allocate(z(3)%f  (bn(1)-1:en(1)+1, bn(2)-1:en(2)+1, bn(3)-1:en(3)+1, 0:q))
allocate(z(3)%ft (bn(1)-1:en(1)+1, bn(2)-1:en(2)+1, bn(3)-1:en(3)+1, 0:q))
allocate(z(3)%feq(bn(1)-1:en(1)+1, bn(2)-1:en(2)+1, bn(3)-1:en(3)+1, 0:q))
...
if(isDebug) then
	if(masterId) then
		write(*,*) "MPI Periodicity",period
		write(*,*) "----------------------"
	endif

	do i=0,nproc-1
		if(i==id) then
			write(*,'(A,I3,3(A,I5,I5))') "bs-es : ",id," | ",bs(1),es(1)," | ",bs(2),es(2)," | ",bs(3),es(3)
			write(*,'(A,I3,3(A,I5,I5))') "bn-en : ",id," | ",bn(1),en(1)," | ",bn(2),en(2)," | ",bn(3),en(3)
			write(*,'(A,I3,3(A,I10  ))') "mySpan: ",id," | ",      pP(1)," | ",      pP(2)," | ",      pP(3)
		endif
	enddo
	...
	if(masterId) 	write(*,*) "Master Proc(s): ",id
	if(eastId)		write(*,*) "East   Proc(s): ",id
	if(westId) 		write(*,*) "West   Proc(s): ",id
	if(northId) 	write(*,*) "North  Proc(s): ",id
	if(southId) 	write(*,*) "South  Proc(s): ",id
	if(topId) 		write(*,*) "Top    Proc(s): ",id
	if(bottomId) 	write(*,*) "Bottom Proc(s): ",id
...
outermost: do k=bn(3),en(3)
	do j=bn(2),en(2)
		do i=bn(1),en(1)
			...
			tmp1 = (i-z(3)%centr(1))/Chord
			if(tmp1 .ge. zer .and. tmp1 .le. one) tmp2 = Chord*naca0012Shape(tmp1)

			if(tmp1 .ge. zer .and. tmp1 .le. one .and. abs(k-z(3)%centr(3)) .le. tmp2)	then
				sphereId = .true.
				exit outermost
			endif
...
if(isDebug .and. sphereId) write(*,'(A,I4,A)') "Processor ",id," has the object"
!-----------------------------------------------------------------------
if(sphereId) then
	...
	allocate(isn(z(1)%np(1),z(1)%np(2),z(1)%np(3)))
endif
!----------------------------------------------------------------------
	...
	do j=bs(2),es(2) 
		do i=bs(1),es(1)
			do a=0,q
				tmp1				= uInlet*ex(a)
				tmp2				= uInlet*uInlet
				z(3)%f (i,j,k,a)	= wt(a)*rho0*(1.0 + 3.0*tmp1 + 4.5*tmp1*tmp1 - 1.5*tmp2)!commenting this line causes non-zero Cl,Cs. Why?
				z(3)%ft(i,j,k,a)	= wt(a)*rho0*(1.0 + 3.0*tmp1 + 4.5*tmp1*tmp1 - 1.5*tmp2)
			enddo
		enddo
	enddo
enddo
		...
		do j=1,z(1)%np(2)
			do i=1,z(1)%np(1)
				...
				do a=0,q
					...
					z(1)%ftA(i,j,k,a,2) = wt(a)*rho0*(1.0 + 3.0*tmp1 + 4.5*tmp1*tmp1 - 1.5*tmp2)
					z(1)%ftA(i,j,k,a,1) = wt(a)*rho0*(1.0 + 3.0*tmp1 + 4.5*tmp1*tmp1 - 1.5*tmp2)
				enddo
...
!---------------------------Detect Sphere-------------------------------
	...
	do k=2,z(1)%np(3)-1
		do j=2,z(1)%np(2)-1
			do i=2,z(1)%np(1)-1
				...
				tmp1 = (i-z(1)%centr(1))/(m(1)*Chord)
				if(tmp1 .ge. zer .and. tmp1 .le. one) tmp2 = m(1)*Chord*naca0012Shape(tmp1)! 

				if(tmp1 .ge. zer .and. tmp1 .le. one .and. abs(k-z(1)%centr(3)) .le. tmp2)	then
					isn(i,j,k) = 1
					ii=ii+1
				else
					isn(i,j,k) = 0
	...
	if(isDebug) write(*,'(A,I10,A)') "The object has ",ii," points within"
...
do ts=1,time
	...
	if(sphereId .and. mod(ts-1,freqDisp)==0) call cpu_time(tStart)
	
	do k=bn(3),en(3) !2,nz-1 !Streaming
		do j=bn(2),en(2) !2,ny-1
			do i=bn(1),en(1) !2,nx-1
			
				if(i>(z(2)%leftFace(1)+1) .and. i<(z(2)%rightFace(1)-1) .and. &
					 j>(z(2)%leftFace(2)+1) .and. j<(z(2)%rightFace(2)-1) .and. &
					 k>(z(2)%leftFace(3)+1) .and. k<(z(2)%rightFace(3)-1)) cycle
				...
				do a=0,q
					...
					z(3)%f(ia,ja,ka,a) = z(3)%ft(i,j,k,a)
				enddo
			enddo
		enddo
	enddo
	!---------------------------------------------------------------------
	call mpi_sendrecv(z(3)%f(bn(1),bn(2),en(3)+1,0),1,xy1pP,des(3),50,z(3)%f(bn(1),bn(2),bn(3),0),1,xy1pP,src(3),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1),bn(2),bn(3)-1,0),1,xy1pM,src(3),50,z(3)%f(bn(1),bn(2),en(3),0),1,xy1pM,des(3),50,mpi_comm_world,STATUS,ierr)

	call mpi_sendrecv(z(3)%f(bn(1),en(2)+1,bn(3),0),1,xz1pP,des(2),50,z(3)%f(bn(1),bn(2),bn(3),0),1,xz1pP,src(2),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1),bn(2)-1,bn(3),0),1,xz1pM,src(2),50,z(3)%f(bn(1),en(2),bn(3),0),1,xz1pM,des(2),50,mpi_comm_world,STATUS,ierr)

	call mpi_sendrecv(z(3)%f(en(1)+1,bn(2),bn(3),0),1,yz1pP,des(1),50,z(3)%f(bn(1),bn(2),bn(3),0),1,yz1pP,src(1),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1)-1,bn(2),bn(3),0),1,yz1pM,src(1),50,z(3)%f(en(1),bn(2),bn(3),0),1,yz1pM,des(1),50,mpi_comm_world,STATUS,ierr)
	!----------------------------------------------------------------------
	call mpi_sendrecv(z(3)%f(bn(1),en(2)+1,en(3)+1,15),1,linex,yz(1),50,z(3)%f(bn(1),bn(2),bn(3),15),1,linex,yz(3),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1),bn(2)-1,en(3)+1,16),1,linex,yz(2),50,z(3)%f(bn(1),en(2),bn(3),16),1,linex,yz(4),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1),bn(2)-1,bn(3)-1,18),1,linex,yz(3),50,z(3)%f(bn(1),en(2),en(3),18),1,linex,yz(1),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1),en(2)+1,bn(3)-1,17),1,linex,yz(4),50,z(3)%f(bn(1),bn(2),en(3),17),1,linex,yz(2),50,mpi_comm_world,STATUS,ierr)
	
	call mpi_sendrecv(z(3)%f(en(1)+1,bn(2),en(3)+1,11),1,liney,xz(1),50,z(3)%f(bn(1),bn(2),bn(3),11),1,liney,xz(3),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1)-1,bn(2),en(3)+1,12),1,liney,xz(2),50,z(3)%f(en(1),bn(2),bn(3),12),1,liney,xz(4),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1)-1,bn(2),bn(3)-1,14),1,liney,xz(3),50,z(3)%f(en(1),bn(2),en(3),14),1,liney,xz(1),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(en(1)+1,bn(2),bn(3)-1,13),1,liney,xz(4),50,z(3)%f(bn(1),bn(2),en(3),13),1,liney,xz(2),50,mpi_comm_world,STATUS,ierr)

	call mpi_sendrecv(z(3)%f(en(1)+1,en(2)+1,bn(3),7 ),1,linez,xy(1),50,z(3)%f(bn(1),bn(2),bn(3),7 ),1,linez,xy(3),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1)-1,en(2)+1,bn(3),8 ),1,linez,xy(2),50,z(3)%f(en(1),bn(2),bn(3),8 ),1,linez,xy(4),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(bn(1)-1,bn(2)-1,bn(3),10),1,linez,xy(3),50,z(3)%f(en(1),en(2),bn(3),10),1,linez,xy(1),50,mpi_comm_world,STATUS,ierr)
	call mpi_sendrecv(z(3)%f(en(1)+1,bn(2)-1,bn(3),9 ),1,linez,xy(4),50,z(3)%f(bn(1),en(2),bn(3),9 ),1,linez,xy(2),50,mpi_comm_world,STATUS,ierr)	
!-----------------------------------------------------------------------
!~ 	if(southId) then
!~ 		j=2 
!~ 		do k=bn(3),en(3) !2,nz-1
!~ 			do i=bn(1),en(1) !2,nx-1
!~ 				z(3)%f(i,j,k,3 )  = z(3)%f(i,j-1,k,4 )  
!~ 				z(3)%f(i,j,k,7 )  = z(3)%f(i,j-1,k,9 )  
!~ 				z(3)%f(i,j,k,8 )  = z(3)%f(i,j-1,k,10)  
!~ 				z(3)%f(i,j,k,15)  = z(3)%f(i,j-1,k,16)  
!~ 				z(3)%f(i,j,k,17)  = z(3)%f(i,j-1,k,18)  
!~ 			enddo
!~ 		enddo
!~ 	endif
...
!~ 		do k=bn(3),en(3) !2,nz-1
!~ 			do i=bn(1),en(1) !2,nx-1
!~ 				z(3)%f(i,j,k,4 )  = z(3)%f(i,j+1,k,3 )  
!~ 				z(3)%f(i,j,k,9 )  = z(3)%f(i,j+1,k,7 )  
!~ 				z(3)%f(i,j,k,10)  = z(3)%f(i,j+1,k,8 )  
!~ 				z(3)%f(i,j,k,16)  = z(3)%f(i,j+1,k,15)  
!~ 				z(3)%f(i,j,k,18)  = z(3)%f(i,j+1,k,17)  
!~ 			enddo
!~ 		enddo
!~ 	endif
	...
	if(bottomId) then
		...
		do j=bn(2),en(2) !2,ny-1
			do i=bn(1),en(1) !2,nx-1
				z(3)%f(i,j,k,5 )  = z(3)%f(i,j,k-1,6 ) 
				z(3)%f(i,j,k,11)  = z(3)%f(i,j,k-1,13)  
				z(3)%f(i,j,k,12)  = z(3)%f(i,j,k-1,14)  
				z(3)%f(i,j,k,15)  = z(3)%f(i,j,k-1,17)  
				z(3)%f(i,j,k,16)  = z(3)%f(i,j,k-1,18)  
			enddo
		enddo
	endif
	...
	if(topId) then
		...
		do j=bn(2),en(2) !2,ny-1
			do i=bn(1),en(1) !2,nx-1
				z(3)%f(i,j,k,6 )  = z(3)%f(i,j,k+1,5 ) 
				z(3)%f(i,j,k,13)  = z(3)%f(i,j,k+1,11)  
				z(3)%f(i,j,k,14)  = z(3)%f(i,j,k+1,12)  
				z(3)%f(i,j,k,17)  = z(3)%f(i,j,k+1,15)  
				z(3)%f(i,j,k,18)  = z(3)%f(i,j,k+1,16)  
			enddo
		enddo
	endif
	...
	if(westId) then
		...
		do k=bn(3),en(3) !2,nz-1
			do j=bn(2),en(2) !2,ny-1
				...
				tmp2 = zer
				do a=0,q
					if (ex(a) == 	0)	tmp2 = tmp2 + z(3)%f(i,j,k,a)
					if (ex(a) == -1)	tmp1 = tmp1 + z(3)%f(i,j,k,a) 
				enddo
				
				rho = (tmp2 + 2.0*tmp1)/(1.0-uInlet) 
				z(3)%f  (i,j,k,1 )  = z(3)%f(i,j,k,kb(1 ))  + (1.0/3.0)*(rho*uInlet)
				z(3)%f  (i,j,k,7 )  = z(3)%f(i,j,k,kb(7 ))  - (quar*(z(3)%f(i,j,k,3)  - z(3)%f(i,j,k,4) )) + (1.0/6.0)*(rho*uInlet)
				z(3)%f  (i,j,k,9 )  = z(3)%f(i,j,k,kb(9 ))  + (quar*(z(3)%f(i,j,k,3)  - z(3)%f(i,j,k,4) )) + (1.0/6.0)*(rho*uInlet)
				z(3)%f  (i,j,k,11)  = z(3)%f(i,j,k,kb(11))  - (quar*(z(3)%f(i,j,k,5)  - z(3)%f(i,j,k,6) )) + (1.0/6.0)*(rho*uInlet)  
				z(3)%f  (i,j,k,13)  = z(3)%f(i,j,k,kb(13))  + (quar*(z(3)%f(i,j,k,5)  - z(3)%f(i,j,k,6) )) + (1.0/6.0)*(rho*uInlet) 
			enddo
		enddo
	endif
	...
	if(eastId) then
		...
		do k=bn(3),en(3) !2,nz-1
			do j=bn(2),en(2) !2,ny-1
				...
				z(3)%f(i,j,k,kb(1 ))  = z(3)%f(i,j,k,1 )  - (1.0/3.0)*(rho0*ux) 
				z(3)%f(i,j,k,kb(7 ))  = z(3)%f(i,j,k,7 )  + (quar*(z(3)%f(i,j,k,3)  - z(3)%f(i,j,k,4) )) - (1.0/6.0)*(rho0*ux)  
				z(3)%f(i,j,k,kb(9 ))  = z(3)%f(i,j,k,9 )  - (quar*(z(3)%f(i,j,k,3)  - z(3)%f(i,j,k,4) )) - (1.0/6.0)*(rho0*ux)  
				z(3)%f(i,j,k,kb(11))  = z(3)%f(i,j,k,11)  + (quar*(z(3)%f(i,j,k,5)  - z(3)%f(i,j,k,6) )) - (1.0/6.0)*(rho0*ux)    
				z(3)%f(i,j,k,kb(13))  = z(3)%f(i,j,k,13)  - (quar*(z(3)%f(i,j,k,5)  - z(3)%f(i,j,k,6) )) - (1.0/6.0)*(rho0*ux)   
			enddo
		enddo
	endif
!-----------------------------------------------------------------------
	...
	do k=bn(3),en(3) !2,nz-1
		do j=bn(2),en(2) !2,ny-1
			do i=bn(1),en(1) !2,nx-1
				...
				if(i>(z(2)%leftFace(1)) .and. i<(z(2)%rightFace(1)) .and. &
					 j>(z(2)%leftFace(2)) .and. j<(z(2)%rightFace(2)) .and. &
					 k>(z(2)%leftFace(3)) .and. k<(z(2)%rightFace(3))) cycle
				...
				do a=0,q
					rho= rho+ z(3)%f(i,j,k,a) 
					ux = ux + z(3)%f(i,j,k,a) *ex(a)
					uy = uy + z(3)%f(i,j,k,a) *ey(a)
					uz = uz + z(3)%f(i,j,k,a) *ez(a)
				enddo
				ux  = ux/rho
				uy  = uy/rho
				uz  = uz/rho
				...
				do a=0,q
					...
					z(3)%feq(i,j,k,a)  = wt(a)*rho*(1.0 + 3.0*tmp1 + 4.5*tmp1*tmp1 - 1.5*tmp2) 
					z(3)%ft (i,j,k,a)  = z(3)%f(i,j,k,a)  - (z(3)%f(i,j,k,a) -z(3)%feq(i,j,k,a) )/z(3)%tau 
				enddo				
	...
	CALL MPI_Reduce(ii,jj,1,mpi_integer,mpi_sum,0,mpi_comm_world,ierr)
...
!-------Transfer post-collision dist to fine block boundary------------	
	if(sphereId) then
		allocate(xIN (z(2)%span(1)+1)) !hardwired
		allocate(yIN (z(2)%span(2)+1))
		allocate(zIN (z(2)%span(3)+1))
		allocate(xOUT(z(2)%np(1)-2))
		allocate(yOUT(z(2)%np(2)-2))
		allocate(zOUT(z(2)%np(3)-2))

		allocate(fINyz (z(2)%span(2)+1,z(2)%span(3)+1))
		!allocate(fINxz (z(2)%span(1)+1,z(2)%span(3)+1))
		allocate(fINxy (z(2)%span(1)+1,z(2)%span(2)+1))
		allocate(fOUTyz(z(2)%np(2)-2,z(2)%np(3)-2))
		!allocate(fOUTxz(z(2)%np(1)-2,z(2)%np(3)-2))
		allocate(fOUTxy(z(2)%np(1)-2,z(2)%np(2)-2))	
	
		s=1
		do i=2,z(2)%np(1)-1,mRel(2)
			xIN(s)=i
			s=s+1
		enddo
...
!~ 		open(unit=16,file="afterInt.dat")
		...
		do a=0,q
			...
			do k=2,z(2)%np(3)-1,mRel(2)
				do j=2,z(2)%np(2)-1,mRel(2)
					i=2
					tmp1 = z(3)%feq(z(2)%leftFace(1)+(i-2)/mRel(2),z(2)%leftFace(2)+(j-2)/mRel(2),z(2)%leftFace(3)+(k-2)/mRel(2),a)
					tmp2 = z(3)%ft (z(2)%leftFace(1)+(i-2)/mRel(2),z(2)%leftFace(2)+(j-2)/mRel(2),z(2)%leftFace(3)+(k-2)/mRel(2),a)
					z(2)%ftA(i,j,k,a,3) = tmp1 + (z(2)%tau-one)/(mRel(2)*(z(3)%tau-one))*(tmp2-tmp1)
					fINyz((j-2)/mRel(2)+1,(k-2)/mRel(2)+1) = z(2)%ftA(i,j,k,a,3)
				enddo
			enddo
...
!~ 					tmp2 = z(3)%ft (z(2)%leftFace(1)+(i-2)/mRel(2),z(2)%leftFace(2)+(j-2)/mRel(2),z(2)%leftFace(3)+(k-2)/mRel(2),a)
!~ 					z(2)%ftA(i,j,k,a,3) = tmp1 + (z(2)%tau-one)/(mRel(2)*(z(3)%tau-one))*(tmp2-tmp1)
!~ 					fINxz((i-2)/mRel(2)+1,(k-2)/mRel(2)+1) = z(2)%ftA(i,j,k,a,3)
!~ 				enddo
!~ 			enddo	
...
!~ 			enddo	
			...
			do j=2,z(2)%np(2)-1,mRel(2)
				do i=2,z(2)%np(1)-1,mRel(2)
					...
					tmp2 = z(3)%ft (z(2)%leftFace(1)+(i-2)/mRel(2),z(2)%leftFace(2)+(j-2)/mRel(2),z(2)%leftFace(3)+(k-2)/mRel(2),a)
					z(2)%ftA(i,j,k,a,3) = tmp1 + (z(2)%tau-one)/(mRel(2)*(z(3)%tau-one))*(tmp2-tmp1)
					fINxy((i-2)/mRel(2)+1,(j-2)/mRel(2)+1) = z(2)%ftA(i,j,k,a,3)
				enddo
			enddo	
		...
		deallocate(fOUTxy)				
	!-----------------------------------------------------------------------
		do k=2,z(2)%np(3)-1 !copy ft_f(1) to ft_f_
			do j=2,z(2)%np(2)-1
				do i=2,z(2)%np(1)-1
					do a=0,q			
						z(2)%ft(i,j,k,a) = z(2)%ftA(i,j,k,a,2) 
					enddo
				enddo
			enddo
		enddo
		...
		do iter2=1,mRel(2)
				...
				do j=2,z(2)%np(2)-1
					do i=2,z(2)%np(1)-1
						do a=0,q	
							...
							if(ja<2) 						ja = z(2)%np(2)-1
							if(ja>z(2)%np(2)-1)	ja = 2

							z(2)%f(ia,ja,ka,a) = z(2)%ft(i,j,k,a) 
						enddo
					enddo	
				enddo
			enddo
			...
			do k=3,z(2)%np(3)-2
				do j=2,z(2)%np(2)-1
					do i=3,z(2)%np(1)-2
						...
						do a=0,q 
							rho = rho + z(2)%f(i,j,k,a) 
							ux = ux + z(2)%f(i,j,k,a)*ex(a) 
							uy = uy + z(2)%f(i,j,k,a)*ey(a) 
							uz = uz + z(2)%f(i,j,k,a)*ez(a)  
						enddo
						ux 	= ux/rho  
						uy 	= uy/rho  
						uz 	= uz/rho  
						...
						do a=0,q
							tmp1					 = ux*ex(a) + uy*ey(a) + uz*ez(a)  
							tmp2					 = ux**two + uy**two + uz**two 
							z(2)%feq(i,j,k,a) = wt(a)*rho*(1.0d0 + 3.0d0*tmp1 + 4.5d0*tmp1**two - 1.5d0*tmp2) 
							z(2)%ft(i,j,k,a) = z(2)%f(i,j,k,a) - (z(2)%f(i,j,k,a)-z(2)%feq(i,j,k,a))/z(2)%tau 
						enddo						
			...
			rhoAvg = rhoAvg/ii!((z(2)%np(1)-2)*(z(2)%np(2)-2)*(z(2)%np(3)-2)) 
	...
	!-----------------------------west yz-plane-----------------------------
			do k=2,z(2)%np(3)-1
				do j=2,z(2)%np(2)-1
					i=2
					do a=0,q
						yiL(1)=z(2)%ftA(i,j,k,a,1) 
						yiL(2)=z(2)%ftA(i,j,k,a,2) 
						yiL(3)=z(2)%ftA(i,j,k,a,3) 
						z(2)%ft(i,j,k,a) = lagrng3p(xiL,yiL,two+dble(iter2)/mRel(2))
					enddo
				enddo
			enddo	
	!-----------------------------east yz-plane-----------------------------	
...
!~ 				enddo	
		...
		!-----------------------------------------------------------------------
					...
					do j=2,z(1)%np(2)-1
						do i=2,z(1)%np(1)-1
							do a=0,q	
								...
								if(ja<2) 						ja = z(1)%np(2)-1
								if(ja>z(1)%np(2)-1)	ja = 2								
			...
			!-------------------------------------------
				...
				Cd = fx_t/(haf*rho0*uInlet*uInlet*Chord*(z(3)%np(2)-2)) 
				Cs = fy_t/(haf*rho0*uInlet*uInlet*Chord*(z(3)%np(2)-2)) 
				Cl = fz_t/(haf*rho0*uInlet*uInlet*Chord*(z(3)%np(2)-2)) 
				!----------------------------------------------------------------------
				fx(1) = fx(2) 
				fy(1) = fy(2)
				fz(1) = fz(2)
				...
				rhoAvg = rhoAvg/ii!((z(1)%np(1)-4)*(z(1)%np(2)-2)*(z(1)%np(3)-4)) 
...
!~ 				enddo			
		...
		!-----------------------------west yz-plane-----------------------------	
			do k=2+mRel(1),z(1)%np(3)-1-mRel(1),mRel(1)
				do j=2,z(1)%np(2)-1,mRel(1) !removed mRel(1) for cylinder
					i=2+mRel(1)
					do a=0,q
						ii=(z(1)%leftFace(1)  -z(2)%leftFace(1))*mRel(1)+2  +(i-2)/mRel(1)
						jj=(z(1)%leftFace(2) -z(2)%leftFace(2))*mRel(1)+2 +(j-2)/mRel(1)
...
!~ 			enddo	
	...
	!=========Tranfer post-collision dist on coarse block boundary==========
	!-----------------------------west yz-plane-----------------------------	
		do k=2+mRel(2),z(2)%np(3)-1-mRel(2),mRel(2)
			do j=2,z(2)%np(2)-1,mRel(2)
				i=2+mRel(2)
				do a=0,q
					z(3)%ft(z(2)%leftFace(1)+(i-2)/mRel(2),z(2)%leftFace(2)+(j-2)/mRel(2),z(2)%leftFace(3)+(k-2)/mRel(2),a) = z(2)%feq(i,j,k,a) + mRel(2)*(z(3)%tau-1)/(z(2)%tau-1)*(z(2)%ftA(i,j,k,a,3)-z(2)%feq(i,j,k,a)) 
				enddo
			enddo	
		enddo
	!-----------------------------east yz-plane-----------------------------		
...
!~ 	!-----------------------------south xz-plane-----------------------------	
!~ 		do k=2+mRel(2),z(2)%np(3)-1-mRel(2),mRel(2)
!~ 			do i=2+mRel(2),z(2)%np(1)-1-mRel(2),mRel(2)
!~ 				j=2+mRel(2)
!~ 				do a=0,q
!~ 					z(3)%ft(z(2)%leftFace(1)+(i-2)/mRel(2),z(2)%leftFace(2)+(j-2)/mRel(2),z(2)%leftFace(3)+(k-2)/mRel(2),a) = z(2)%feq(i,j,k,a) + mRel(2)*(z(3)%tau-1)/(z(2)%tau-1)*(z(2)%ftA(i,j,k,a,3)-z(2)%feq(i,j,k,a)) 
!~ 				enddo
			...
			do k=2,z(2)%np(3)-1
				do j=2,z(2)%np(2)-1
					do i=2,z(2)%np(1)-1
						...
						write(12,'(3(F8.2,1X),I4,1X,3(F8.4,1X),F15.8)') (z(2)%leftFace(1)+dble(i-2)/m(2)),&
																														(z(2)%leftFace(2)+dble(j-2)/m(2)),&
																														(z(2)%leftFace(3)+dble(k-2)/m(2)),&
...
function naca0012Shape(x) result(y)

double precision,intent(in)::x
double precision::y 

y=5.0d0*0.12d0*(0.2969d0*sqrt(x) - 0.1260d0*x - 0.3516d0*x*x + 0.2843d0*x**3d0 - 0.1015d0*x**4.0d0)

end function naca0012Shape
