...
implicit none
!-----------------------------------------------------------------------
integer,parameter,dimension(3)::	nP			=[299,179,179],	&
																	topo		=[4,1,3]
...
outermost: do k=bn(3),en(3)
	do j=bn(2),en(2)
		do i=bn(1),en(1)
			...
			if (tmp1 .le. haf*D) then !removed m* on 1107
				sphereId = .true.
				exit outermost
			endif
						...
						if (isn(i,j,k) == 0) then
							do a=0,q
								...
								if (isn(ia,ja,ka)==1) then					
									...
									fx(2) = fx(2) + quar*ex(a)*2.0*(-ft_f_(ia,ja,ka,kb(a)) + ft_f_(i,j,k,a)) 						
									fy(2) = fy(2) + quar*ey(a)*2.0*(-ft_f_(ia,ja,ka,kb(a)) + ft_f_(i,j,k,a)) 						
									fz(2) = fz(2) + quar*ez(a)*2.0*(-ft_f_(ia,ja,ka,kb(a)) + ft_f_(i,j,k,a)) 
								endif
							enddo
						endif
				...
				do j=2,ny_f-1
					do i=2,nx_f-1
						write(12,'(3(F8.2,1X),I4,1X,4(F8.4,1X))') (westFace+dble(i)/m),(southFace+dble(j)/m),(bottomFace+dble(k)/m),&
																											0,ux_f(i,j,k)/uInlet,uy_f(i,j,k)/uInlet,uz_f(i,j,k)/uInlet,&
