...
integer,parameter,dimension(3)::	snapList=[10,10000,100000]
...
double precision,parameter:: 	D=10.0d0			,&
															tau_c=1.50d0	,&
															rho0=1.0d0		,&
															uInlet=0.0033d0	,&
...
outermost: do k=bn(3),en(3)
	do j=bn(2),en(2)
		do i=bn(1),en(1)
			...
			tmp1 = ((i-cx_c)**two + (j-cy_c)**two + (k-cz_c)**two)**haf 
...
!if(sphereId) print*,id," is sphereId"
!stop
!-----------------------------------------------------------------------
if(sphereId) then
	...
	allocate(yIN (ny_f/2))
	allocate(zIN (nz_f/2))
	allocate(xOUT(nx_f/2-1))
	allocate(yOUT(ny_f/2-1))
	allocate(zOUT(nz_f/2-1))

	allocate(fINyz (ny_f/2,nz_f/2))
	allocate(fINxz (nx_f/2,nz_f/2))
	allocate(fINxy (nx_f/2,ny_f/2))
	allocate(fOUTyz(ny_f/2-1,nz_f/2-1))
	allocate(fOUTxz(nx_f/2-1,nz_f/2-1))
	allocate(fOUTxy(nx_f/2-1,ny_f/2-1))
	...
	do k=2,nz_f-1
		do j=2,ny_f-1
			do i=2,nx_f-1
				...
				tmp1 = ((i-cx)**two + (j-cy)**two + (k-cz)**two)**haf 
	...
	if(westId) then
		...
		do k=bn(3),en(3) !2,nz-1
			do j=bn(2),en(2) !2,ny-1
				...
				f_c  (i,j,k,7 ) = f_c(i,j,k,kb(7 )) - (0.25*(f_c(i,j,k,3) - f_c(i,j,k,4))) + (1.0/6.0)*(rho_c(i,j,k)*uInlet)
				f_c  (i,j,k,9 ) = f_c(i,j,k,kb(9 )) + (0.25*(f_c(i,j,k,3) - f_c(i,j,k,4))) + (1.0/6.0)*(rho_c(i,j,k)*uInlet)
				f_c  (i,j,k,11) = f_c(i,j,k,kb(11)) - (0.25*(f_c(i,j,k,5) - f_c(i,j,k,6))) + (1.0/6.0)*(rho_c(i,j,k)*uInlet)  
				f_c  (i,j,k,13) = f_c(i,j,k,kb(13)) + (0.25*(f_c(i,j,k,5) - f_c(i,j,k,6))) + (1.0/6.0)*(rho_c(i,j,k)*uInlet) 
			enddo
		enddo
	endif
	...
	if(eastId) then
		...
		do k=bn(3),en(3) !2,nz-1
			do j=bn(2),en(2) !2,ny-1
				...
				f_c (i,j,k,kb(7 )) = f_c(i,j,k,7 ) + (0.25*(f_c(i,j,k,3) - f_c(i,j,k,4))) - (1.0/6.0)*(rho0*ux_c(i,j,k))  
				f_c (i,j,k,kb(9 )) = f_c(i,j,k,9 ) - (0.25*(f_c(i,j,k,3) - f_c(i,j,k,4))) - (1.0/6.0)*(rho0*ux_c(i,j,k))  
				f_c (i,j,k,kb(11)) = f_c(i,j,k,11) + (0.25*(f_c(i,j,k,5) - f_c(i,j,k,6))) - (1.0/6.0)*(rho0*ux_c(i,j,k))    
				f_c (i,j,k,kb(13)) = f_c(i,j,k,13) - (0.25*(f_c(i,j,k,5) - f_c(i,j,k,6))) - (1.0/6.0)*(rho0*ux_c(i,j,k))   
			enddo
		enddo
	endif
!-----------------------------------------------------------------------
	...
	if (masterId .and. mod(ts,freqDisp) == 0) then
		rhoAvgAll =rhoAvgAll/(product(nP-2)-(eastFace-westFace-3)*(northFace-southFace-3)*(topFace-bottomFace-3))
...
!-------Transfer post-collision dist to fine block boundary------------	
	if(sphereId) then
		...
		do a=0,q
			...
			do k=2,nz_f-2,m
				do j=2,ny_f-2,m
					i=2
					ft_f(i,j+1,k+1,a,3)=fOUTyz(j/m,k/m) !hardwired
				enddo
			enddo
			!-----------------------------east yz-plane-------------------------
			do k=2,nz_f-1,m
			...
			do k=2,nz_f-2,m
				do i=2,nx_f-2,m
					j=2
					ft_f(i+1,j,k+1,a,3)=fOUTxz(i/m,k/m) !hardwired
				enddo
			enddo
			!---------------------north xz-plane--------------------------------
			do k=2,nz_f-1,m
			...
			do j=2,ny_f-2,m
				do i=2,nx_f-2,m
					k=2
					ft_f(i+1,j+1,k,a,3)=fOUTxy(i/m,j/m) !hardwired
				enddo
			enddo	
			!-------------------------top xy-plane------------------------------
			do j=2,ny_f-1,m
	...
	!-----------------------------------------------------------------------
		...
		do iter=1,m
			...
			rhoAvg = rhoAvg/((nx_f-2)*(ny_f-2)*(nz_f-2)) 
	!----------------------------------------------------------------------
				...
				do j=3,ny_f-2
					do k=3,nz_f-2
						...
						do a=0,q
							...
							feq_f(i,j,k,a) = wt(a)*rho_f(i,j,k)*(1.0 + 3.0*tmp1 + 4.5*tmp1*tmp1 - 1.5*tmp2) 
...
!=======================================================================
		...
		if(sphereId) then
	!-------------------------------------1---------------------------------
			write(12,'(3(A,I6))') "Zone I= ",(westFace+1)-bn(1)+1,	&
															 " ,J= ",en(2)-bn(2)+1,			&
															 " ,K= ",en(3)-bn(3)+1

			do k=bn(3),en(3) !2,nz-1
				do j=bn(2),en(2) !2,ny-1
					do i=bn(1),westFace+1
						write(12,'(4(I4,1X),4(F8.4,1X))') i,j,k,0,ux_c(i,j,k),uy_c(i,j,k),uz_c(i,j,k),rho_c(i,j,k) 
					enddo
					write(12,*)
				enddo
				write(12,*)
...
!-----------------------------------7-----------------------------------	
			...
			do k=2,nz_f-1
				do j=2,ny_f-1
					do i=2,nx_f-1
						write(12,'(3(F8.2,1X),I4,1X,4(F8.4,1X))') (westFace+dble(i)/m),(southFace+dble(j)/m),(bottomFace+dble(k)/m),&
																											 isn(i,j,k),ux_f(i,j,k),uy_f(i,j,k),uz_f(i,j,k),rho_f(i,j,k) 
					enddo
					write(12,*)
				enddo
				write(12,*)
