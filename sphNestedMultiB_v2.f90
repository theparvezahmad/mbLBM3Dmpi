...
integer,parameter,dimension(1)::	snapList=[1]
...
integer,dimension(3)					::np,leftFace,rightFace,centr
end type zone
...
!=======================================================================
z(3)%np=[299,179,179]
z(2)%np=m(2)*span(2)+3 !includes 2 buffer nodes
z(1)%np=m(1)*span(1)+3
...
z(1:2)%leftFace (1)	= z(3)%centr(1)-span(1:2)/2
z(1:2)%rightFace(1)	= z(3)%centr(1)+span(1:2)/2
z(1:2)%leftFace (2)	= z(3)%centr(2)-span(1:2)/2
z(1:2)%rightFace(2)	= z(3)%centr(2)+span(1:2)/2
z(1:2)%leftFace (3)	= z(3)%centr(3)-span(1:2)/2
z(1:2)%rightFace(3)	= z(3)%centr(3)+span(1:2)/2
!==============================MPI======================================
CALL mpi_init(ierr)
...
outermost: do k=bn(3),en(3)
	do j=bn(2),en(2)
		do i=bn(1),en(1)
			...
			tmp1 = ((i-z(3)%centr(1))**two + (j-z(3)%centr(2))**two + (k-z(3)%centr(3))**two)**haf 
	...
	do j=bs(2),es(2) 
		do i=bs(1),es(1)
			do a=0,q
				tmp1				= uInlet*ex(a)
				tmp2				= uInlet*uInlet
				z(3)%f (i,j,k,a)	= wt(a)*rho0*(1.0 + 3.0*tmp1 + 4.5*tmp1*tmp1 - 1.5*tmp2)!commenting this line causes non-zero Cl,Cs
	...
	do k=2,z(1)%np(3)-1
		do j=2,z(1)%np(2)-1
			do i=2,z(1)%np(1)-1
				...
				tmp1 = ((i-z(1)%centr(1))**two + (j-z(1)%centr(2))**two + (k-z(1)%centr(3))**two)**haf 
...
!-----------------------------------------------------------------------
	...
	if (masterId .and. mod(ts,freqDisp) == 0) then
		rhoAvgAll =rhoAvgAll/(product(z(3)%np-2)-(z(2)%rightFace(1)-z(2)%leftFace(1)-1)*(z(2)%rightFace(2)-z(2)%leftFace(2)-1)*(z(2)%rightFace(3)-z(2)%leftFace(3)-1))
		write(*,'(A,I8,F8.4)') "Rho(Coarse) ",	ts, rhoAvgAll
	endif
!-------Transfer post-collision dist to fine block boundary------------	
	if(sphereId) then
		allocate(xIN (span(2)+1)) !hardwired
		allocate(yIN (span(2)+1))
		allocate(zIN (span(2)+1))
		allocate(xOUT(z(2)%np(1)-2))
		allocate(yOUT(z(2)%np(2)-2))
		allocate(zOUT(z(2)%np(3)-2))

		allocate(fINyz (span(2)+1,span(2)+1))
		allocate(fINxz (span(2)+1,span(2)+1))
		allocate(fINxy (span(2)+1,span(2)+1))
		allocate(fOUTyz(z(2)%np(2)-2,z(2)%np(3)-2))
		allocate(fOUTxz(z(2)%np(1)-2,z(2)%np(3)-2))
		allocate(fOUTxy(z(2)%np(1)-2,z(2)%np(2)-2))	
	
		s=1
		do i=2,z(2)%np(1)-1,m(2)
			xIN(s)=i
			s=s+1
		enddo
...
!~ 		open(unit=16,file="afterInt.dat")
		...
		do a=0,q
			...
			do k=2,z(2)%np(3)-1,m(2)
				do j=2,z(2)%np(2)-1,m(2)
					i=2
					tmp1 = z(3)%feq(z(2)%leftFace(1)+(i-2)/m(2),z(2)%leftFace(2)+(j-2)/m(2),z(2)%leftFace(3)+(k-2)/m(2),a)
					tmp2 = z(3)%ft (z(2)%leftFace(1)+(i-2)/m(2),z(2)%leftFace(2)+(j-2)/m(2),z(2)%leftFace(3)+(k-2)/m(2),a)
					z(2)%ftA(i,j,k,a,3) = tmp1 + (z(2)%tau-one)/(m(2)*(z(3)%tau-one))*(tmp2-tmp1)
					fINyz((j-2)/m(2)+1,(k-2)/m(2)+1) = z(2)%ftA(i,j,k,a,3)
				enddo
			enddo
			...
			do k=2,z(2)%np(3)-1,m(2)
				do i=2,z(2)%np(1)-1,m(2)
					...
					tmp2 = z(3)%ft (z(2)%leftFace(1)+(i-2)/m(2),z(2)%leftFace(2)+(j-2)/m(2),z(2)%leftFace(3)+(k-2)/m(2),a)
					z(2)%ftA(i,j,k,a,3) = tmp1 + (z(2)%tau-one)/(m(2)*(z(3)%tau-one))*(tmp2-tmp1)
					fINxz((i-2)/m(2)+1,(k-2)/m(2)+1) = z(2)%ftA(i,j,k,a,3)
				enddo
			enddo	
			...
			do j=2,z(2)%np(2)-1,m(2)
				do i=2,z(2)%np(1)-1,m(2)
					...
					tmp2 = z(3)%ft (z(2)%leftFace(1)+(i-2)/m(2),z(2)%leftFace(2)+(j-2)/m(2),z(2)%leftFace(3)+(k-2)/m(2),a)
					z(2)%ftA(i,j,k,a,3) = tmp1 + (z(2)%tau-one)/(m(2)*(z(3)%tau-one))*(tmp2-tmp1)
					fINxy((i-2)/m(2)+1,(j-2)/m(2)+1) = z(2)%ftA(i,j,k,a,3)
				enddo
			enddo	
	...
	!-----------------------------------------------------------------------
		...
		do iter2=1,mRel(2)
			...
			rhoAvg = rhoAvg/((z(2)%np(1)-2)*(z(2)%np(2)-2)*(z(2)%np(3)-2)) 
...
!~ 			stop
			...
			do k=2,z(2)%np(3)-1
				do j=2,z(2)%np(2)-1
					...
					do a=0,q
						...
						z(2)%ft(i,j,k,a) = lagrng3p(xiL,yiL,two+dble(iter2)/m(2))
					enddo
				enddo
			enddo	
	!-----------------------------east yz-plane-----------------------------	
	...
	!------------------------------top xy-plane-----------------------------		
			...
			allocate(xIN (mRel(1)*span(1)+1)) !hardwired
			allocate(yIN (mRel(1)*span(1)+1))
			allocate(zIN (mRel(1)*span(1)+1))
			allocate(xOUT(z(1)%np(1)-2))
			allocate(yOUT(z(1)%np(2)-2))
			allocate(zOUT(z(1)%np(3)-2))

			allocate(fINyz (mRel(1)*span(1)+1,mRel(1)*span(1)+1))
			allocate(fINxz (mRel(1)*span(1)+1,mRel(1)*span(1)+1))
			allocate(fINxy (mRel(1)*span(1)+1,mRel(1)*span(1)+1))
